...
def create_excel(start_row, end_row, input_df, output_file):
    ...
    if pd.notna(first_start_date):
        sheet_name = f'NH_{first_start_date.strftime("%B_%d_%Y")}'  # Use (Month, Day, Year) format for sheet name
    else:
    ...
    center_alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)  # Wrap text in cells
    bold_font = Font(size=14, bold=True)  # Bold font for headers
    regular_font = Font(size=14)  # Regular font for other cells

    # Apply styles to the header row
    for cell in sheet[1]:
        cell.fill = header_fill
        cell.alignment = center_alignment
        cell.font = bold_font  # Set the header font to bold
    ...
    for column in sheet.columns:
        ...
        for cell in column:
            if cell.value:
                ...
                cell.alignment = center_alignment  # Use the alignment that includes wrap_text
                cell.font = regular_font  # Apply regular font to cells
                cell.border = Border(left=Side(border_style='thin'),
                                     right=Side(border_style='thin'),
                                     top=Side(border_style='thin'),
                                     bottom=Side(border_style='thin'))
    ...
    for row in sheet.iter_rows():
        for cell in row:
            cell.alignment = center_alignment  # Apply the same alignment
            cell.font = regular_font  # Apply regular font to cells
            cell.border = Border(left=Side(border_style='thin'),
                                 right=Side(border_style='thin'),
                                 top=Side(border_style='thin'),
